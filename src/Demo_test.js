...
function blendColors(color1, color2, factor) {
  var or1 = Core__Int.fromString(color1.substring(1, 3), 16);
  var og1 = Core__Int.fromString(color1.substring(3, 5), 16);
  var ob1 = Core__Int.fromString(color1.substring(5, 7), 16);
  var or2 = Core__Int.fromString(color2.substring(1, 3), 16);
  var og2 = Core__Int.fromString(color2.substring(3, 5), 16);
  var ob2 = Core__Int.fromString(color2.substring(5, 7), 16);
  var r1;
  if (or1 !== undefined) {
    r1 = or1;
  } else {
  ...
  var r = r1 * factor + r2 * (1 - factor) | 0;
  var g = g1 * factor + g2 * (1 - factor) | 0;
  var b = b1 * factor + b2 * (1 - factor) | 0;
  return "#" + r.toString(16).padStart(2, "0") + g.toString(16).padStart(2, "0") + b.toString(16).padStart(2, "0");
}

if (blendColors("#FF0088", "#FF0088", 0) !== "#ff0088") {
  throw {
        RE_EXN_ID: "Assert_failure",
        _1: [
          "Demo_test.res",
...
if (blendColors("#000000", "#FF0088", 0) !== "#ff0088") {
  throw {
        RE_EXN_ID: "Assert_failure",
        _1: [
          "Demo_test.res",
...
if (blendColors("#000000", "#FF0088", 1) !== "#000000") {
  throw {
        RE_EXN_ID: "Assert_failure",
        _1: [
          "Demo_test.res",
...
if (blendColors("#000000", "#FF0088", 0.5) !== "#7f0044") {
  throw {
        RE_EXN_ID: "Assert_failure",
        _1: [
          "Demo_test.res",
...
function sum(myArray) {
  return Core__Array.reduce(myArray, 0, (function (prev, cur) {
                return prev + cur | 0;
              }));
}

function randomWeighted(blockTypeWeights) {
  var weights = Belt_Array.map(Belt_Array.concatMany([blockTypeWeights]), (function (v) {
          return v[1];
        }));
  var totalWeight = Core__Array.reduce(weights, 0, (function (prev, cur) {
          return prev + cur | 0;
        }));
  ...
  weights.forEach(function (x, i) {
        var nextVal = cumulativeWeight.contents + x;
        if (nextVal > randomValue && myValue.contents === undefined) {
          myValue.contents = i;
        }
        cumulativeWeight.contents = nextVal;
      });
  var num = myValue.contents;
  if (num === undefined) {
    return weights.length - 1 | 0;
  }
  if (num > (weights.length - 1 | 0)) {
    throw {
          RE_EXN_ID: "Assert_failure",
          _1: [
            "Demo_test.res",
...
var results = Core__Array.make(1000, 0).map(function (_v) {
...
console.log(0, results.filter(function (v) {
          return v === 0;
        }).length);

console.log(1, results.filter(function (v) {
          return v === 1;
        }).length);

console.log(2, results.filter(function (v) {
          return v === 2;
        }).length);

console.log(3, results.filter(function (v) {
          return v === 3;
        }).length);

console.log(4, results.filter(function (v) {
          return v === 4;
        }).length);

console.log(5, results.filter(function (v) {
          return v === 5;
        }).length);

console.log("total", results.length);

function getRandomBlockType2(currentLevel) {
  var blockTypeWeights = [
    [
      "Empty",
      Math.max(10 - currentLevel | 0, 5)
    ],
    [
      "Multiply",
      Math.min(6 + (currentLevel / 2 | 0) | 0, 12)
    ],
    [
      "Remove",
      Math.min(5 + (currentLevel / 3 | 0) | 0, 9)
    ],
    [
      "Diagonal",
      Math.min(6 + currentLevel | 0, 12)
    ],
    [
      "Plus",
      Math.min(5 + (currentLevel / 2 | 0) | 0, 10)
    ],
    [
      "Chevron",
      Math.min(4 + (currentLevel / 2 | 0) | 0, 8)
    ]
  ];
  var index = randomWeighted(blockTypeWeights);
  var match = blockTypeWeights[index];
  if (match !== undefined) {
    return match[0];
  }
  throw {
