...
class Block {
    ...
    constructor(x: number, y: number, width: number, height: number, type: BlockType = "Empty", multiplyValue: number = 0) {
        this.type = type;
    ...
    public isPointInRibbon(x: number, y: number): boolean {
        ...
        if (this.type !== "Multiply" && this.type !== "Remove") return false;
...
class PhysicsGame {
    ...
    private lineSegmentIntersectsRibbon(x1: number, y1: number, x2: number, y2: number, block: Block): boolean {
        if (block.type !== "Multiply" && block.type !== "Remove") return false;
