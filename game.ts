...
class Block {
    ...
    public bounds: { min: { x: number; y: number }; max: { x: number; y: number } };
    ...
    public drawRibbon(ctx: CanvasRenderingContext2D): void {
        ...
        if (this.type === BlockType.Multiply) {
            ctx.save();
            ...
            ctx.fillText(`x${this.value}`,
                centerX,
                ribbonY + ribbonHeight / 2
            );
            ctx.restore();
        } else if (this.type === BlockType.Plus) {
            ctx.save();
            ...
            ctx.restore();
        } else if (this.type === BlockType.Remove) {
    ...
    public isPointInRibbon(x: number, y: number): boolean {
        if (this.type === BlockType.Plus) {
            const ribbonHeight = (this.bounds.max.y - this.bounds.min.y) / 3;
            const ribbonY = (this.bounds.max.y + this.bounds.min.y) / 2 - ribbonHeight / 2;
        ...
        if (this.type !== BlockType.Multiply && this.type !== BlockType.Remove) return false;
...
class PhysicsGame {
    ...
    private static readonly GLOBAL_BOUNCE = 0.7;
    private static readonly GLOBAL_FRICTION = 0;
    ...
    private ballsRemaining: number = 20;
    private initialBallCount: number = 20; // Track initial ball count for spawn rate calculation
    private spawnActive: boolean = false;
    private levelCompleteMessage: string = "";
    private levelCompleteTimer: number = 0;
    private readonly SPAWN_INTERVAL = 300; // Base spawn interval in ms
    private readonly SPAWN_HEIGHT = 50;
    ...
    private readonly BLOCK_COUNT = 5;
    private readonly WALL_THICKNESS = 10;
    private readonly WALL_START_Y = 150;
    private readonly BLOCK_HEIGHT_RATIO = 0.8;
    private readonly DIAGONAL_THICKNESS = 10;
    private readonly LEVEL_COMPLETE_MESSAGE_DURATION = 120;
    private readonly MIN_SPAWN_INTERVAL = 50; // Minimum spawn interval (fastest speed)
    private readonly MAX_BALLS_FOR_BASE_SPEED = 20; // Number of balls below which we use the base speed
    private readonly NO_COLLECTION_TIMEOUT = 300; // 5 seconds at 60 FPS
    private readonly MAX_LEVELS = 3; // Maximum number of levels before game ends
    private verticalWallSegments: Matter.Body[][][] = [];
    
    // Total number of balls for the level (for tracking completion)
    private totalBallsInLevel: number = 20;
    // Timer to track time since last ball was collected
    private timeSinceLastCollection: number = 0;
    ...
    private ballToVisitedBlocks: Map<number, Set<Block>> = new Map();
    ...
    private createLanes(): void {
        ...
        this.verticalWallSegments = Array(this.LANE_COUNT + 1).fill(null).map(() =>
            Array(this.BLOCK_COUNT).fill(null).map(() => [] as Matter.Body[])
        );
        ...
        for (let laneIndex = 0; laneIndex <= this.LANE_COUNT; laneIndex++) {
            ...
            for (let blockIndex = 0; blockIndex < this.BLOCK_COUNT; blockIndex++) {
                ...
                const wallSegment = Bodies.rectangle(
                    x,
                    y,
                    this.WALL_THICKNESS,
                    blockHeight,
            ...
            if (laneIndex === 0 || laneIndex === this.LANE_COUNT) {
                ...
                const sideWallExtension = Bodies.rectangle(
                    x,
                    extraWallY,
                    this.WALL_THICKNESS,
                    extraWallHeight,
    ...
    private createBlocks(): void {
        ...
        const blockLayout: BlockType[][] = this.generateRandomBlockLayout();

        for (let laneIndex = 0; laneIndex < this.LANE_COUNT; laneIndex++) {
            const laneArray: Block[] = [];
            ...
            for (let blockIndex = 0; blockIndex < this.BLOCK_COUNT; blockIndex++) {
                ...
                if (block.type !== BlockType.Empty && block.type !== BlockType.Multiply &&
                    block.type !== BlockType.Remove && block.body !== null) {
    ...
    private clearExistingBlocks(): void {
    ...
    private getRandomBlockType(weights: Record<BlockType, number>): BlockType {
        ...
        for (const type in weights) {
            totalWeight += weights[Number(type) as BlockType];
        }
        ...
        for (const type in weights) {
            cumulativeWeight += weights[Number(type) as BlockType];
            if (randomValue < cumulativeWeight) {
                return Number(type) as BlockType;
            }
        }
    ...
    private createDiagonalWall(block: Block, laneIndex: number, blockIndex: number) {
        if (block.diagonalWall) {
            return;
        }

        // Check for adjacent diagonal block that could create a V-trap
        let adjacentDirection: 'leftToRight' | 'rightToLeft' | null = null;
        ...
        if (laneIndex > 0 && blockIndex < this.BLOCK_COUNT) {
            ...
            if (leftLaneBlocks && Array.isArray(leftLaneBlocks) && leftLaneBlocks.length > blockIndex) {
                ...
                if (leftBlock && leftBlock.type === BlockType.Diagonal && leftBlock.diagonalDirection) {
        ...
        if (laneIndex < this.LANE_COUNT - 1 && blockIndex < this.BLOCK_COUNT) {
            ...
            if (rightLaneBlocks && Array.isArray(rightLaneBlocks) && rightLaneBlocks.length > blockIndex) {
                ...
                if (rightBlock && rightBlock.type === BlockType.Diagonal && rightBlock.diagonalDirection) {
        ...
        const diagonalBody = Bodies.fromVertices(
            centerX, centerY,
            [vertices],
            {
                isStatic: true,
    ...
    private drawDiagonalForBlock(block: Block, ctx: CanvasRenderingContext2D) {
        if (block.type !== BlockType.Diagonal || !block.bounds) {
            return;
        }
    ...
    private removeWallSegment(laneIndex: number, blockIndex: number) {
    ...
    private findBlockByBody(body: Matter.Body): Block | null {
    ...
    private handleBlockCollision(ball: Matter.Body, block: Block): void {
        switch (block.type) {
            case BlockType.Multiply:
            ...
            case BlockType.Remove:
                ...
                if (block.counter <= 0) {
                    ...
                    for (const laneArray of this.blocks) {
                        ...
                        if (blockIndex !== -1) {
                            laneArray[blockIndex] = new Block(
                                blockBounds.x,
                                blockBounds.y,
                                blockBounds.width,
                                blockBounds.height,
    ...
    private drawMultiplyEffect(x: number, y: number, value: number = 2): void {
    ...
    private drawPlusEffect(x: number, y: number, value: number = 2): void {
        const ctx = this.render.context;
        ctx.save();
    ...
    private drawRemoveEffect(x: number, y: number): void {
        const ctx = this.render.context;
        ctx.save();
    ...
    private createBall(): Matter.Body {
        ...
        if (this.spawnActive && currentTime > this.nextSpawnTime && this.ballsRemaining > 0) {
            ...
            if (this.initialBallCount > this.MAX_BALLS_FOR_BASE_SPEED) {
                ...
                spawnInterval = Math.max(
                    this.SPAWN_INTERVAL - (extraBalls * scalingFactor),
    ...
    private removeRandomWalls(): void {
    ...
    private checkRibbonCollisions(ball: Matter.Body): void {
            ...
            laneArray.forEach((block, blockIndex) => {
                ...
                if (!collisionDetected && (Math.abs(ball.velocity.x) > 2 || Math.abs(ball.velocity.y) > 2)) {
                    ...
                    if (!collisionDetected) {
                        collisionDetected = this.lineSegmentIntersectsRibbon(
                ...
                if (block.type === BlockType.Multiply &&
                ...
                else if (block.type === BlockType.Plus && collisionDetected) {
                ...
                else if (block.type === BlockType.Remove && collisionDetected) {
                    block.counter--;
                    block.hitAnimTimer = 10;
                    ...
                    if (block.counter <= 0) {
                        const newBlock = new Block(
                            (block.bounds.min.x + block.bounds.max.x) / 2,
                            (block.bounds.min.y + block.bounds.max.y) / 2,
    ...
    private lineSegmentIntersectsRibbon(x1: number, y1: number, x2: number, y2: number, block: Block): boolean {
        if (block.type !== BlockType.Multiply && block.type !== BlockType.Remove) return false;
    ...
    private handleMultiplication(ball: Matter.Body, block: Block): void {
        ...
        for (let i = 0; i < newBallsToCreate; i++) {
            ...
            const newBall = Bodies.circle(
                ball.position.x + positionOffsetX,
                ball.position.y + positionOffsetY,
                this.BALL_RADIUS,
                {
                    restitution: PhysicsGame.GLOBAL_BOUNCE,
                    friction: PhysicsGame.GLOBAL_FRICTION,
    ...
    private handlePlusBlock(ball: Matter.Body, block: Block): void {
